...
const routeManifestPath = path.join(runtimeDir, 'rust-rsc-route-manifest.json')
...
if (fs.existsSync(prettier)) {
  execFileSync(prettier, ['--write', routeManifestPath], { stdio: 'ignore' })
}
  ...
  .map((route) => {
    ...
    for (const layoutPath of route.layouts) {
      ...
      const slotInitializers = Object.keys(route.parallelSlots)
        .map(
          (name) =>
            `                props.slots.insert(${JSON.stringify(name)}.to_owned(), crate::next_layout_router(${JSON.stringify(name)}));`
        )
        .join('\n')
      branches.push(`            ${JSON.stringify(requestKey)} => {
                let ${Object.keys(route.parallelSlots).length > 0 ? 'mut ' : ''}props = LayoutProps::new(crate::next_layout_router("children"), params.clone()).with_request(request.clone());
${slotInitializers}
    ...
    for (let index = 1; index <= pathnameParts.length; index++) {
      ...
      if (!componentKeys.has(requestKey)) {
        branches.push(
          `            ${JSON.stringify(requestKey)} => return Some(Ok(crate::next_layout_router("children"))),`
        )
      }
    }
...
const parallelSlotBranches = nativeRoutes
  ...
  .join('\n')
const generatedRoutesPath = path.join(runtimeDir, 'src', 'generated_routes.rs')
fs.writeFileSync(
  generatedRoutesPath,
...
execFileSync(process.env.RUSTFMT || 'rustfmt', [
  '--edition',
  '2024',
  generatedRoutesPath,
])
...
function readNextInternalClientReferences() {
  ...
  if (!fs.existsSync(manifestPath)) {
    throw new Error(
      `Native App Router payloads require a production client reference manifest: ${manifestPath}`
    )
  }
  const manifest = readClientReferenceManifest(manifestPath)
  const layoutRouter = findClientReference(
    manifest,
  ...
  if (!layoutRouter || !templateContext) {
    throw new Error(
      `Native App Router payloads require LayoutRouter and RenderFromTemplateContext in ${manifestPath}`
    )
  }
  return { layoutRouter, templateContext }
}
...
function findClientReference(manifest, suffix, moduleName, exportName) {
  const match = Object.entries(manifest.clientModules || {}).find(([module]) =>
    module.replaceAll('\\', '/').endsWith(suffix)
  )
  ...
  if (!Array.isArray(reference.chunks) || reference.id == null) {
    throw new Error(
      `Incomplete ${bundler} client reference metadata for ${moduleName}`
    )
  }
  return {
    module: moduleName,
