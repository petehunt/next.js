...
Status: implemented prototype; all written phase gates satisfied for their declared scope
...
- Framework-owned discovery now runs the embedded-Wasm bridge for Rust root/nested/dynamic layouts, pages, named slots, `loading.rs`, `not-found.rs`, and `error.rs`. Webpack chains Rust → SWC → Flight so the boundary enters the client manifest; a config-time prepass materializes Turbopack's loader-renamed client identity before graph construction. The error client adapter invokes Wasm in the browser with the live message/digest and exposes Next's reset callback as an opaque Rust-placeable slot. The generated e2e fixture passes all four focused dev/production Webpack/Turbopack modes and proves the Rust error boundary catches a dynamic Server Component failure.
...
- Edge bridge execution now uses deployment-precompiled Wasm: `pub const RUNTIME: &str = "edge"` becomes Next's segment runtime export, and a compile-time DCE-safe runtime split loads a `?module` Wasm sidecar through Next's Edge Wasm loader instead of dynamically compiling base64 bytes. A real Rust Edge page passed development and production in both Webpack and Turbopack; Node keeps its embedded reusable instance path.
- `next-rsc-loader/compiler.js` now owns compilation and `.next/cache/rust-rsc` artifact identity inside Next rather than the playground loader. The adapter refuses to compile without that injected framework interface. Cache identity includes source, SDK, harness, target/profile, and `rustc -vV`; atomic file locks coalesce processes. A focused run measured 1,148 ms first compile, 126 ms cached load, and 1,103 ms changed-source compile while also proving SDK/toolchain invalidation and lock cleanup. Compiled route discovery proves the disabled flag excludes `.rs`, and real Webpack/Turbopack requests cover both a Wasm panic with its source path and an ordinary returned `RenderError`.
